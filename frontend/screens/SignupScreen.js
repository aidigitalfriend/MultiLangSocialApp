...
const LANGUAGES = [
  ...
  { code: 'th', label: 'Thai' }, { code: 'tr', label: 'Turkish' }, { code: 'vi', label: 'Vietnamese' },
  { code: 'nl', label: 'Dutch' }, { code: 'sv', label: 'Swedish' }, { code: 'pl', label: 'Polish' },
  { code: 'id', label: 'Indonesian' }, { code: 'ms', label: 'Malay' }, { code: 'fil', label: 'Filipino' },
];
  ...
  const handleSignup = async () => {
    ...
    try {
      const response = await fetch('https://api.v4u.ai/signup', {
        ...
        body: JSON.stringify({ phone, username, language, password }),
      });
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.input} onPress={() => setShowLangPicker(!showLangPicker)}>
            <Text style={{ color: DARK }}>{selectedLang?.label || 'Select language'}</Text>
          </TouchableOpacity>
          {showLangPicker && (
            <View style={styles.langGrid}>
              {LANGUAGES.map(l => (
                <TouchableOpacity key={l.code} style={[styles.langChip, language === l.code && styles.langChipActive]}
...
const styles = StyleSheet.create({
  ...
  langGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 8, marginBottom: 8 },
  langChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: BG, borderWidth: 1, borderColor: '#DDD' },
